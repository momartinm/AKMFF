...

{
  ...
  for ( f = ginitial; f; f = f->next ) {
    ...
    if ( !lneg[lp][adr] ) {/* non deleted ini */
      ...
      /* DELETE GOALS*/
      /* ADDED FOR TEST WITH ABSTRACT DOMAIN */
      /* Moisés Martínez UC3M*/
        
      static_tmp = new_Facts();
      static_tmp->fact->predicate = lp;
      
      for ( j = 0; j < garity[lp]; j++ ) 
      {
	static_tmp->fact->args[j] = f->fact->args[j];
      }
      
      static_tmp->next = gstatic;
      gstatic = static_tmp;
      gnum_static++;
