...

{
  ...
  while ( ( h != 0 ) && (gcmd_line.horizon > gnum_plan_ops))
  {
...

{
  ...
  if ( lH ) {
    for ( i = 0; i < gnum_H; i++ ) {
      if ( result_to_dest( &S__, S, gH[i], 10000000 ) ) {
	add_to_ehc_space( &S__, gH[i], NULL );
      }
    }
  } else {
    for ( i = 0; i < gnum_A; i++ ) {
      if ( result_to_dest( &S__, S, gA[i], 10000000 ) ) {
...

{
  ...
  if ( lH ) {
    for ( i = 0; i < gnum_H; i++ ) {
      if ( result_to_dest( &S_, &(lehc_current_start->S), gH[i], 0 ) ) {
	add_to_ehc_space( &S_, gH[i], lehc_current_start );
      }
    }
  } else {
    for ( i = 0; i < gnum_A; i++ ) {
      if ( result_to_dest( &S_, &(lehc_current_start->S), gA[i], 0 ) ) {
...

{
  ...
  while (TRUE)
  {
   ...
   for ( i = 0; i < first->num_H; i++ ) 
	{
      if ( result_to_dest( &S, &(first->S), first->H[i], first->level ) ) 
		{
 ...
 */
Bool result_to_dest( State *dest, State *source, int op, int horizon )

{
  int type = gop_conn[op].action->norm_operator->operator->type;

  if ((horizon < gcmd_line.horizon) && (type > 1))
  {
	return FALSE;
  }
  if ((horizon >= gcmd_line.horizon) && (type == 1))
  ...
  if ( gcmd_line.display_info ) 
  {
	printf("OPERATOR: %s(%d)\n", gop_conn[op].action->name, gop_conn[op].action->norm_operator->operator->type);
  }
