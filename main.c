...
PlNode *new_APlNode(PlNode *node, char *predicate)
{
	PlNode *new_son = NULL;
	PlNode *sons = NULL;
	PlNode *son = NULL;
	PlNode *new_node = NULL;

	if (node->connective == ATOM)
	{
		if (strcmp(node->atom->item, predicate) != 0)
		{
			new_node = new_PlNode(node->connective);
			new_node->parse_vars = node->parse_vars;
			new_node->atom = node->atom;
			new_node->comp = node->comp;
			new_node->neft = node->neft;
			new_node->lh = node->lh;
			new_node->rh = node->rh;
			new_node->sons = NULL;
	...
	{
		for ( son = node->sons; son; son = son->next )
		{
			new_son = new_APlNode(son, predicate);
			if (new_son != NULL)
			{		
				new_son->next = sons;
				sons = new_son;
...
PlOperator *new_AOperator(PlOperator *operator, char *predicate)
{
  PlNode *pln = NULL;
  PlNode *pln_new = NULL;  

  char * name = new_Token(strlen(operator->name)+5);
  sprintf(name, "%s%s", operator->name, "-ABS");

  PlOperator *node = new_PlOperator(name);

  TypedList *params = NULL;
  TypedList *temp = NULL;
  TypedList *first = NULL; 
  TypedList *last = NULL;
	  
  for (params = operator->parse_params; params; params = params->next) 
  {
	temp = new_TypedList();

	temp->name = new_Token( strlen(params->name)+1 );
	strcpy( temp->name, params->name); 

	temp->type = new_TokenList();
	temp->type->item = new_Token( strlen (params->type->item)+1 );
	strcpy(temp->type->item, params->type->item);
  ...
  node->parse_params = first;

  node->number_of_real_params = operator->number_of_real_params;
  node->type     = 2; 
  node->next 	 = NULL;

  for ( pln = operator->preconds; pln; pln = pln->next )
  {
     pln_new = new_APlNode(pln, predicate);
     if (pln_new != NULL)
     {
	pln_new->next = node->preconds;
	node->preconds = pln_new;
     }
  }

  for ( pln = operator->effects; pln; pln = pln->next )
  {
     pln_new = new_APlNode(pln, predicate);
     
     if (pln_new != NULL)
     {
	pln_new->next = node->effects;
	node->effects = pln_new;
     }
  }

  if ((node->effects == NULL) || (node->preconds == NULL))
  {
	free( node->name );
    	free_FactList( node->params );
    	/*free_PlNode( node->preconds );
    	free_PlNode( node->effects );*/
...

{
  ...
  PlOperator *po;
  PlOperator *operator = NULL;

  int numAbstractOperators = 0;
  ...
  for ( po = gloaded_ops; po; po = po->next)
  {
	if (po->type == 1)
	{
		operator = new_AOperator(po, gcmd_line.predicate);
		if (operator != NULL)
		{	
			operator->next = gloaded_ops;
			gloaded_ops = operator;

			numAbstractOperators++;
		}
	}
  }

  printf("ff: Generated %d abstract operators using predicate %s\n", numAbstractOperators, gcmd_line.predicate);
  ...
  if ( found_plan ) {
    /*print_plan();*/
    printf("ff: Plan generated successfully in file (plan_final.tmp)\n");
  }

  save_plan("plan_final.tmp");
...
void output_planner_data( void )
{
  FILE *result;
  
  if ((result = fopen( "result.txt", "a")) == NULL ) 
  {
    printf("\n\nCan't open result output file!\n\n");
    return;
  }
  fprintf(result, "%s;%s;%d;%7.2f;%7.2f;%d;%d\n", gcmd_line.ops_file_name, gcmd_line.fct_file_name, gcmd_line.horizon, gtempl_time + greach_time + grelev_time + gLNF_time + gconn_time + gsearch_time, gsearch_time, gevaluated_states, gnum_plan_ops );
...
void ff_usage( void )
{
  ...
  printf("-h <num>    set weight w_h in w_g*g(s) + w_h*h(s) [preset: %d]\n\n",
	 gcmd_line.h_weight);
  printf("-k <num>    horizon where abstraction will be applicated\n");
  printf("-a <str>    predicate will be eliminated to build the abstraction\n"); 
...
Bool process_command_line( int argc, char *argv[] )
{
  ...
  while ( --argc && ++argv ) {
	...
	case 'h':
	  ...
	  break;
        case 'k':
          sscanf( *argv, "%d", &gcmd_line.horizon );

	  if (gcmd_line.horizon == -1)
	  {
			gcmd_line.plan 		= TRUE;
			gcmd_line.horizon 	= 1000000;
	  }
	  else
	  {
			if (gcmd_line.horizon == 0)
				gcmd_line.horizon = 1000000;
		
			gcmd_line.plan 	= FALSE;
	  }

          break;  
        case 'a':
	  strncpy( gcmd_line.predicate, *argv, MAX_LENGTH );
	  for (i=0; i<strlen(gcmd_line.predicate); i++)
	  {
          	gcmd_line.predicate[i] = toupper(gcmd_line.predicate[i]);
	  } 
	  break;    
	default:
