...
void print_hidden_TokenList_File( TokenList *list, char *sep, FILE * out )
...
int is_Predicate(char * name)
{
	int find = 0;

	TypedListList *auxiliar = gparse_predicates;

	while ((auxiliar != NULL) && (!find))
	{
		find = (strcmp(auxiliar->predicate, name) == 0) ? 1:0;
...
void print_Fact_File( Fact *f , FILE * out)

{
  ...
  /*aux = malloc(4);
  strncpy(aux, gpredicates[f->predicate], 4);
*/
  if (is_Predicate(gpredicates[f->predicate]))
  {
...
void print_ft_name_File( int index, FILE *out )

{

  print_Fact_File( &(grelevant_facts[index]), out );
...
void print_op_name_file( int index, FILE *out)
...
void printFluentName(char * string, FILE *out)
{
	int i = 0;
...
void print_final_State( State S )

{
  ...
  if ((out = fopen( "state.pddl", "w")) == NULL ) 
  {
  ...
  fprintf( out, "(define (problem %s) (:domain %s)\n", gproblem_name, gdomain_name);
  fprintf( out, "(:objects\n");
  
  strcpy(aux, gparse_objects->type->item);
  
  for ( t = gparse_objects; t; t = t->next ) 
  {

      if (strcmp(aux, t->type->item) != 0)
      {
          fprintf( out, "- %s\n", aux);
          strcpy(aux, t->type->item);          
      }
  ...
  for ( f = gstatic; f; f = f->next ) 
  {
      if (f->fact->predicate != 0)
      {
        print_Fact_File( f->fact, out );
      }
  }
  
  for ( i = 0; i < S.num_F; i++ ) 
  {    
    print_ft_name_File( S.F[i] , out );
  }
  ...
  for ( i = 0; i < gnum_relevant_fluents; i++ ) 
  {
    ...
    {
	if (strstr(grelevant_fluents_name[i], "MINUS") == NULL)
	{
	    	fprintf( out, "(= ");

		printFluentName(grelevant_fluents_name[i], out);
	
		if ( S.f_D[i] )
	    	  	fprintf( out, " %.2f)", S.f_V[i]);
	    	else
	     		fprintf( out, "UNDEF");
  ...
  for ( g = gorig_goal_facts; g; g = g->next ) 
  {
      if ( g->connective == AND ) 
      {
          fprintf( out, "(AND\n");
          
          for ( s = g->sons; s!=NULL; s = s->next ) 
          {
                fprintf(out, "(");
                print_hidden_TokenList_File(s->atom, " ", out);
                fprintf(out, ")\n");
          }
		...
		{
			fprintf(out, "(");
         print_hidden_TokenList_File(g->atom, " ", out);
         fprintf(out, ")\n");
		}	
  }
...
void save_plan( char * name )
{
	...
	else
	{
		if ((out = fopen( name, "w")) == NULL ) 
  		{
    			printf("\n\nCan't create official output file!\n\n");
    			return;
  		}
	}
